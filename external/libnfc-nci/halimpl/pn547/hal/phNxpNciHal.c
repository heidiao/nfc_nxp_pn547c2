...
static void phNxpNciHal_check_factory_reset(void)
{
    ...
    else if(stat(config_eseinfo_path, &st) == 0)
    {
        size = st.st_size;
        NXPLOG_NCIHAL_D("Size of \"%s\" is %d bytes.\n", config_eseinfo_path, size);
        file_ptr = fopen(config_eseinfo_path, "rb");
        if (file_ptr)
        {
            recvdBytesSize = fread(buffer,1, MAX_FILE_SIZE, file_ptr);
            NXPLOG_NCIHAL_D("recvdBytesSize = %d \n",recvdBytesSize);
            for(i = 0;i < 6;++i)
            {
                //NXPLOG_NCIHAL_D("%02X ", ((unsigned char *)buffer)[i]);
                NXPLOG_NCIHAL_D("%02X ", ((unsigned char *)buffer)[STATIC_PIPE_INDEX + i]);
            }
        }
        else
        {
            // error opening file
            NXPLOG_NCIHAL_E("Error in Opening file");
            close(file_ptr);
        }
        if((((unsigned char *)buffer)[STATIC_PIPE_INDEX] == 00 ) && (((unsigned char *)buffer)[STATIC_PIPE_INDEX +1] == 00) &&
           (((unsigned char *)buffer)[STATIC_PIPE_INDEX +2] == 00) && (((unsigned char *)buffer)[STATIC_PIPE_INDEX +3] == 00) &&
           (((unsigned char *)buffer)[STATIC_PIPE_INDEX +4] == 00) && (((unsigned char *)buffer)[STATIC_PIPE_INDEX +5] == 00))
        {
        ...
        {
            NXPLOG_NCIHAL_E("/data/nfc/nfaStorage.bin1 == Contents already exist ");
        }
    }
