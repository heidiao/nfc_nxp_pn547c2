...
NFCSTATUS phDnldNfc_InitImgInfo(void)
{
    ...
    NXPLOG_FWDNLD_D("FW Image Info Pointer - pImageInfo %p",(intptr_t)pImageInfo);
   ...
   if(NFCSTATUS_SUCCESS == wStatus)
   {
       ...
       if((NULL != gpphDnldContext->nxp_nfc_fw) && (0 != gpphDnldContext->nxp_nfc_fw_len))
       {
           ...
           NXPLOG_FWDNLD_D("FW Image Info Pointer - %p",(intptr_t)pImageInfo);
...
NFCSTATUS phDnldNfc_LoadRecInfo(void)
{
   ...
   if(NFCSTATUS_SUCCESS == wStatus)
   {
       ...
       if((NULL != gpphDnldContext->nxp_nfc_fwp) && (0 != gpphDnldContext->nxp_nfc_fwp_len))
       {
           ...
           NXPLOG_FWDNLD_D("Recovery Image Info Pointer - %p",(intptr_t)pImageInfo);
           wStatus = NFCSTATUS_SUCCESS;
       }
       else
       {
...
NFCSTATUS phDnldNfc_LoadPKInfo(void)
{
   ...
   if(NFCSTATUS_SUCCESS == wStatus)
   {
       ...
       if((NULL != gpphDnldContext->nxp_nfc_fwp) && (0 != gpphDnldContext->nxp_nfc_fwp_len))
       {
           ...
           NXPLOG_FWDNLD_D("PKU Image Info Pointer - %p",(intptr_t)pImageInfo);
           wStatus = NFCSTATUS_SUCCESS;
       }
       else
       {
